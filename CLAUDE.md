...
5. 広告は `_layouts/post.html` が `_includes/ad.html` を自動的に記事の先頭と末尾に挿入するので、記事ファイル側で広告を意識する必要はない。広告ネットワークのスクリプトを設定する際は `_includes/ad.html` の中身だけを書き換える（`index.html` や `ai-safety-2025.html` には広告ユニット（`<ins class="adsbygoogle">` など見た目のある広告枠）を追加しない）。

   例外: Google AdSense はサイト所有権の確認にトップページ（ルートドメイン）をクロールするため、`index.html` の `<head>` には AdSense のローダースクリプト（`<script async src="https://pagead2.googlesyndication.com/...">` のみ、広告枠 `<ins>` は無し）を置いている。これは広告表示のためではなく検証専用の例外なので、`ai-safety-2025.html` には適用しない。

   **AdSense ポリシー上の注意**: 「実質的なコンテンツのないページに広告を出してはいけない」というルールがある（ナビゲーション専用ページ、工事中ページなど）。`docs/blog/index.html`（記事一覧ページ）は本文コンテンツを持たないため、front matter に `no_ads: true` を設定して広告を止めている（`_layouts/post.html` 側で `{% unless page.no_ads %}` により広告 include をスキップする仕組み）。同様に低コンテンツなページを追加する場合は `no_ads: true` を検討すること。
